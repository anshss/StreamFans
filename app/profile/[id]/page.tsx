...
function FollowComponent({
  ...
  async function startFlow(xReceiverAddress: string, xFlowRate: string) {
    ...
    console.log("started");
    console.log(`https://app.superfluid.finance/dashboard/${xReceiverAddress}`);
  }
  ...
  return (
    <>
      {publications?.map((pub: any, index: number) => (
        <ShowPublication decryptPublication={decryptPublication} key={index} pub={pub}/>
      ))}
    </>
  );
}
...
function ShowPublication ({pub, decryptPublication}: {pub: any, decryptPublication: any}){
    ...
    return <div  className="py-4 bg-zinc-900 rounded mb-3 px-4">
